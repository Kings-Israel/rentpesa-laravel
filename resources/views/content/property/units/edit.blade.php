...
@section('content')
  <h4 class="fw-bold">
    <span class="text-muted fw-light"><a href="{{ route('properties.index') }}">Units</a> /</span> Edit Unit / {{ $unit->unit_number }}
  </h4>
  <!-- Unit Listing Wizard -->
  <div class="card">
    <div class="card-body">
        ...
        <div id="unit-details" class="content">
          <div class="row g-3">
            <div class="col-sm-6">
              ...
              <select id="property_id" name="property_id" class="select2 form-select" data-allow-clear="true">
                ...
                @foreach($properties as $property)
                  <option value="{{ $property->id }}" @if($unit->property_id == $property->id) selected @endif>{{ $property->name }}</option>
                @endforeach
              </select>
              @error('property_id')
            ...
            <div class="col-sm-6">
              ...
              <input type="text" id="unit_number" name="unit_number" class="form-control" placeholder="Enter Unit Number" value="{{ $unit->unit_number }}" />
              @error('unit_number')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
            <div class="col-sm-6">
              ...
              <select id="unit_type" name="unit_type_id" class="select2 form-select" data-allow-clear="true">
                ...
                @foreach($unit_types as $unit_type)
                  <option value="{{ $unit_type->id }}" @if($unit->unit_type_id == $unit_type->id) selected @endif>{{ $unit_type->name }}</option>
                @endforeach
              </select>
              @error('unit_type_id')
                <span class="text-danger">{{ $message }}</span>
            ...
            <div class="col-sm-6">
              ...
              <select id="billing_frequency" name="billing_frequency_id" class="select2 form-select" data-allow-clear="true">
                ...
                @foreach($billing_frequencies as $billing_frequency)
                  <option value="{{ $billing_frequency->id }}" @if($unit->billing_frequency_id == $billing_frequency->id) selected @endif>{{ $billing_frequency->name }}</option>
                @endforeach
              </select>
              @error('billing_frequency_id')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
            <div class="col-sm-6">
              <label class="form-label" for="floor_number">Floor Number</label>
              <input type="text" id="floor_number" name="floor_number" class="form-control" placeholder="Enter Road/Street" value="{{ $unit->floor_number }}" />
              @error('floor_number')
              <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
            <div class="col-sm-6">
              ...
              <input type="number" id="rent" name="rent" class="form-control" placeholder="10000" value="{{ $unit->rent }}" />
              @error('rent')
              <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
            <div class="col-lg-6">
              ...
              <input type="number" id="deposit" name="deposit" class="form-control" placeholder="12" value="{{ $unit->deposit }}" />
              @error('deposit')
                <span class="text-danger">{{ $message }}</span>
              @enderror
            </div>
            <div class="col-12 d-flex justify-content-between mt-4">
              ...
              <button type="submit" class="btn btn-primary btn-next"> <span class="align-middle d-sm-inline-block d-none me-sm-1">Update</span> <i class="ti ti-arrow-right ti-xs"></i></button>
            </div>
          </div>
        </div>
      </form>
